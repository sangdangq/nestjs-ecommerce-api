import { Controller, Post, Body, Res, HttpStatus, Get, Param, Put, Delete, HttpException } from '@nestjs/common';
...
export class CommentController {
    ...
    async getCommentByProductId(@Param('id') productId: string, @Res() res) {
        ...
        res.status(HttpStatus.BAD_REQUEST).send('Failed to get comment for product');
    }
    ...
    async getCommentByCustomerId(@Param('id') customerId: string, @Res() res) {
        ...
        .catch(err => {
            throw new HttpException('Failed to get comment for customer', HttpStatus.BAD_REQUEST);
        });
    }
